				...
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="起拍价" prop="startprice">
							<el-input class="list_inp" v-model.number="form.startprice" placeholder="起拍价"
								 type="number" 								:readonly="!isAdd||disabledForm.startprice?true:false" />
						</el-form-item>
					</el-col>

					<el-col :span="12">
						<el-form-item label="加价幅度" prop="stepprice">
							<el-input class="list_inp" v-model.number="form.stepprice" placeholder="加价幅度"
								 type="number" 								:readonly="!isAdd||disabledForm.stepprice?true:false" />
						</el-form-item>
					</el-col>

					<el-col :span="12">
						<el-form-item label="一口价" prop="endprice">
							<el-input class="list_inp" v-model.number="form.endprice" placeholder="一口价"
								 type="number" 								:readonly="!isAdd||disabledForm.endprice?true:false" />
						</el-form-item>
					</el-col>
...
<script setup>
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.shangpinxiangqing = res.data.data.shangpinxiangqing?(res.data.data.shangpinxiangqing.replace(reg,'../../../cl8813975/file')):'';
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='startprice'){
					form.value.startprice = row[x];
					disabledForm.value.startprice = true;
					continue;
				}
				if(x=='stepprice'){
					form.value.stepprice = row[x];
					disabledForm.value.stepprice = true;
					continue;
				}
				if(x=='endprice'){
					form.value.endprice = row[x];
					disabledForm.value.endprice = true;
					continue;
				}
			}
			if(row){
			...
			form.value.stepprice='1'
			form.value.endprice='0'
			formVisible.value = true
		}

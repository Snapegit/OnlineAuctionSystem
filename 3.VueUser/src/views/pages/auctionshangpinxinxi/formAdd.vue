	...
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0","margin":"20px auto 60px","borderRadius":"20px","background":"#fff","width":"64%","position":"relative","height":"100%"}'>
		...
		<el-form ref="formRef" :model="form" class="add_form" label-width="120px" :rules="rules">
			<el-row>
				...
				<el-col :span="12">
					<el-form-item label="竞拍价格" prop="auctionprice">
						<el-input class="list_inp" v-model.number="form.auctionprice" placeholder="竞拍价格"
							 type="number" 							:readonly="!isAdd||disabledForm.auctionprice?true:false" />
					</el-form-item>
				</el-col>

				<el-col :span="12">
					<el-form-item label="竞拍状态" prop="auctionstatus">
						<el-select
							class="list_sel"
							:disabled="!isAdd||disabledForm.auctionstatus?true:false"
							v-model="form.auctionstatus" 
							placeholder="请选择竞拍状态"
							style="width:100%;"
							>
							<el-option v-for="(item,index) in auctionstatusLists" :label="item"
								:value="item"
								>
							</el-option>
						</el-select>
...
<script setup>
	...
	//form表单
	const form = ref({
        refid: context?.$toolUtil.storageGet("auctionRefid"),
		userid: '',
		nickname: '',
		avatarurl: '',
		auctionstatus: '',
	...
	const init = (formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null) => {
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='auctionprice'){
					form.value.auctionprice = row[x];
					disabledForm.value.auctionprice = true;
					continue;
				}
				if(x=='auctionstatus'){
					form.value.auctionstatus = row[x];
					disabledForm.value.auctionstatus = true;
					continue;
				}
			}
			if(row){
